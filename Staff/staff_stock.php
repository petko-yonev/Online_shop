...
$url_filter_param = str_replace("http://localhost/Staff/staff_stock.php?stock=$stock&page=$page_num", "", $url);
...
//Create filter
include_once "../PHP_shop/filter.php";
            ...
            while($row = mysqli_fetch_assoc($result)){
                ...
                if($quantity != 0){
                    if(isset($_SESSION["add_to_order"])){
                        echo'<form action = "staff_add_stock_to_order.php" method="POST">
                            ...
                            <input class = "add_to_cart_btn_view_mode" name = "add_to_order_selected_stock" type="submit" value= "Добави към № '.$_SESSION["add_to_order"].'">
                        </form>';
                    } else {
                        echo'<div class = "info_line_a" >Не е избрана поръчка!</div>';
                    }
                } else {
